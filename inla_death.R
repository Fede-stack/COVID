...
xtable::xtable(model.inla$summary.fixed[-c(2:51), ], type = "latex")
xtable::xtable(summary(model.inla)['hyperpar']$hyperpar, type = "latex")

fixeds = data.frame(model.inla$summary.fixed)[-c(1:51), ]
ggplot(fixeds) +
  geom_point(mapping = aes(y = rownames(fixeds), x = mean)) +
  geom_point(mapping = aes(y = rownames(fixeds), x = X0.025quant), col = 'navyblue') +
  geom_point(mapping = aes(y = rownames(fixeds), x = X0.975quant), col = 'navyblue') +
  geom_vline(xintercept = 0, lwd = 1, col = 'indianred', lty = 2) 
...
fixed.eff = names(model.inla$marginals.fixed)[22:27]
...
for(i in 1:length(fixed.eff)) {
  ...
  p[[i]] = ggplot(marginal, aes(x = x, y = y)) + 
    ...
    theme_minimal() +
    ggtitle(paste('Posterior distribution of', fixed.eff[i], 'dummy variable'))
}
...
ggplot(data = R, aes(x = Predictions, y = `Real Values`)) +
  ...
  geom_text(x=5, y=30, label=paste('Adjusted R2: ', round(R2.adj, 3)), cex = 6, col = 'indianred') +
  theme_minimal()
