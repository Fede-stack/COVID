...
# Values from deceduti time series are cumulated, we are looking for the number of new deceduti day by day
if (target == 'deceduti') {
  dates = unique(df$data)[-1]
  df = df %>%
    dplyr::select(data, denominazione_regione, deceduti) %>%
    spread(key = denominazione_regione, value = deceduti) %>%
    summarise(across(.cols = Abruzzo:Veneto, .fns = diff)) %>%
    mutate(data = dates) %>%
    gather(key = 'denominazione_regione', 'target', -data) %>%
    arrange(data)
  target = 'target'
}
...
real_pred = D %>%
  filter(data > tail(D$data, 1)-n.pred) #save the real values of target variables for the last n.pred days
...
demos = read.csv("demos.csv", sep=";", check.names = F)
demos_reshape = demos %>% 
  gather(FASCIA) %>% 
...
inla_data = inla_data %>% left_join(demos_age, by = 'denominazione_regione')
...
omicron = sapply(inla_data$data, function(x) ifelse(x > as.Date('2021-12-20'), 1, 0))
...
PUGLIA = sapply(inla_data$denominazione_regione, function(x) ifelse(x == 'PUGLIA', 1, 0))
TOSCANA = sapply(inla_data$denominazione_regione, function(x) ifelse(x == 'TOSCANA', 1, 0))
...
formula.inla = target ~ 1 + f(ID.area, model="bym2",graph=Italy.adj,scale.model=TRUE,
                               constr=TRUE,hyper = list(prec = list(prior = "pc.prec", param = c(1,0.01)))) + 
  ns(day, df = 50) +
  f(vacc_smooth, model = 'rw2') +
  start + old + booster + omicron.start + year +
  fascia.20_40*year + fascia.40_50*year + fascia.50_60*year + fascia.60_70*year + fascia.70_100 +
  fascia.70_100*old + fascia.70_100*year + fascia.70_100*omicron.start +
  fascia.40_50*booster + fascia.50_60*booster + fascia.60_70*booster + fascia.70_100 *booster
...
(bars = ggplot(df_bar, aes(x = denominazione_regione, y= value, fill = variable)) +
...
# grid.arrange(bars, sc2, layout_matrix = rbind(c(1),c(1), c(2)))
...
sc2 = ggplot(df_4_plot) +
  ...
  xlab('Date') + 
  ylab('New-Daily Death') + 
  # ggtitle(paste('Number of New-Daily Death Across Time in Italy')) +
  geom_point(data = df_4_plot %>%
               filter(data > tail(D$data, 1)-n.pred),
