...
R$Predictions = unlist(sapply(1:7, function(i) ifelse(R[i, 2] > R[i, 3], R[i, 2] - (R[i, 2] - R[i, 3])/1.6, R[i, 2] + (R[i, 3] - R[i, 2])/1.6) ))
mod = lm(`Real Values` ~ Predictions, R)
R2.adj = summary(mod)$r.squared 
ggplot(data = R, aes(x = Predictions, y = `Real Values`)) +
  geom_smooth(method = "lm", se=FALSE, formula = y ~ x ) +
  geom_point() +
  ...
  geom_text(x=450, y=400, label=paste('R squared: ', round(R2.adj, 2)), cex = 6, col = 'indianred') +
  theme_minimal()
