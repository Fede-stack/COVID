...
library(maptools) # for reading shape file of Canton Ticino
...
target       <- "terapia_intensiva" # choose var
...
D = df %>%
  ...
  summarise(terapia_intensiva = sum(terapia_intensiva))  #to calculate the sum of P.A.s
D = D %>%
  filter(data < as.Date('2022-04-17'))

if(sum(D$terapia_intensiva<0)>0) D = D %>% 
  mutate(terapia_intensiva = sapply(D$terapia_intensiva, function(x) ifelse(x<0, 0, x)))
...
population = read_excel("pop.xlsx")
...
inla_data = inla_data %>%
  mutate(ID.day = day, 
         ID.day1 = day, 
         terapia_intensiva = replace(terapia_intensiva, data > tail(D$data, 1)-n.pred, NA), #insert NA for prediction purpose
         y = terapia_intensiva/E, #scelgo come target il rapporto tra le terapie intensive e la popolazione
...
rename_regions_2 = sapply(V$reg, function(x) ifelse(grepl('Provincia', x), 'TRENTINO ALTO ADIGE',
...
Vacc = V %>%
  mutate(denominazione_regione = rename_regions_2, 
         nuovi_vaccinati = apply(V %>%
                                   dplyr::select(m, f), 1, sum), 
         totale_vaccini_somministrati = cumsum(nuovi_vaccinati)) %>%
  group_by(data, eta, denominazione_regione) %>%
  summarise(prima_dose = sum(d1), seconda_dose = sum(d2), dose_addizionale_booster = sum(db1),
            dose_booster2 = sum(db2)) %>%#, nuovi_vaccinati = sum(nuovi_vaccinati), 
            # totale_vaccini_somministrati = sum(prima_dose ))
  mutate(nuovi_vaccinati = prima_dose + seconda_dose + dose_addizionale_booster + dose_booster2)
...
andamento_vacc_fascia_eta = spread(Vacc %>% dplyr::select(data, denominazione_regione, eta, nuovi_vaccinati), 
                                   key = eta, value = nuovi_vaccinati) 
...
colnames(andamento_vacc_fascia_eta)
...
colnames(si_vax)[13:22] = paste('cumulata_fascia', 1:10, sep = '')
...
dosi = Vacc %>%
  ...
  mutate(dosi = prima_dose_sum + dose_booster_sum, data = data+14) %>%
  dplyr::select(dosi, denominazione_regione, data)

inla_data = inla_data %>% left_join(dosi) #%>% dplyr::select(-data)
# fast_handle_na(inla_data)

ggplot(inla_data, mapping = aes(data, dosi)) +
  geom_point() +
  geom_smooth()
...
ggplot(inla_data, mapping = aes(dosi_log, terapia_intensiva)) +
  geom_hex(bins = 66)

inla_data$vacc_smooth =  inla.group(inla_data$dosi_log, n = 50, 'quantile')

set.seed(1)
formula.inla = terapia_intensiva ~ 1 + f(ID.area,model="bym2",graph=Italy.adj,scale.model=TRUE,
                                         constr=TRUE,hyper = list(prec = list(prior = "pc.prec", param = c(1,0.01)))) +
  f(ID.area2,day,model="iid")  +
  # f(ID.day, model = "rw2", scale.model = TRUE,
  #   hyper = list(theta = list(prior = "pc.prec", param = c(1, 0.01)))) +
  ns(ID.day, df = 50) +
  # f(vacc_smooth, model = 'rw2') +
  fascia.20_40 +
  fascia.40_50 + fascia.50_60 + fascia.60_70 + fascia.70_100 + 
  fascia.70_100*old

model.inla = inla(formula.inla,
                  family="poisson",
                  data=inla_data,
                  E=inla_data$E, 
...
# Posterior of precision of the Zero-inflated poisson  
(plot2 = ggplot(as.data.frame(model.inla$marginals.hyperpar[[1]]), aes(x = x, y = y)) + 
    geom_line(col = 'aquamarine') +
    geom_area(fill = 'aquamarine2', alpha = .3) +
    ylab (expression(paste(pi, "(", tau, " | ", bold(y), ")")))  +
    ggtitle('Posterior of Zero-Inflated Poisson') )

set.seed(1)
prec.samp = inla.hyperpar.sample(10000, model.inla)
head(prec.samp)

plot2 +
  geom_histogram(data.frame(sample = prec.samp[, 1]), mapping = aes(x = sample, y = ..density..),
                 bins = 30, alpha = .2, fill = 'blue2')

# PREDICTIONS -------------------------------------------------------------

# prediction of n.pred days
to.pred    <- which(is.na(inla_data$terapia_intensiva))
...
predictions = real_pred %>%
  dplyr::select(denominazione_regione, pred, terapia_intensiva)


# VISUALIZATION -----------------------------------------------------------

df_bar = reshape2::melt(predictions %>% rename(Predictions = pred, 
                                               `Real Values` = terapia_intensiva)  %>% 
                          mutate(data = as.factor(data)), id.vars = c('data', 'denominazione_regione'))

ggplot(df_bar, aes(x = denominazione_regione, y= value, fill = variable)) +
...
fitted = round(rep(pop$E,length(model.inla$summary.fitted.values$mean)/length(pop$E))*model.inla$summary.fitted.values$mean)
real_v = inla_data$terapia_intensiva

ggplot(data.frame(fitted = fitted, real = real_v), mapping = aes(fitted, real)) +
  geom_smooth(method = "lm", se=FALSE, formula = y ~ x ) +
  geom_point() +
  stat_cor(label.y = 1000) +
  # scale_color_hue(l=40, c=35) +
  stat_regline_equation(label.y = 800) +
  # geom_text(x=10, y=60, label=paste('Adjusted R2: ', round(R2.adj, 3)), cex = 6, col = 'indianred') +
  theme_minimal() +
  xlab('Fitted Values') +
  ylab('Real Values')
...
y_italy = D %>%
  group_by(data) %>%
  summarise(y_italy = sum(terapia_intensiva))
...
y_region = D %>%
  filter(denominazione_regione == chosen) %>%
  group_by(data) %>%
  summarise(y_region = sum(terapia_intensiva))
...
sc2 = ggplot(df_4_plot) +
  ...
  xlab('Date') +
  ylab('Intensive Care') + 
  # ggtitle(paste('Number of Intensive Car Occupacy Across Time in Italy')) +
  geom_point(data = df_4_plot %>%
               filter(data > tail(D$data, 1)-n.pred),
